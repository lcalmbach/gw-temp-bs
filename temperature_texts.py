...
""",
'surface_temperature_compare_to_groundwater':"""Figure 4 compares annual average temperatures of ground and surface water in a scatter plot. This representation shows, that groundwater temperatures are approximately 4 degrees warmer than the surface temperature. There is also a poor correlation between the two parameters. Each marker represents a year and the marker fillcolor ranging from blue = early years to red = later years allow appreciate a trend from lower left to upper right. Note that the during the years 2010 to 2014 very few groundwater temperaturess were recorded and therefore the yearly average values may not be very representative. This may explain the high groundwater value of 16°C for the year 2011. In addition, such a correlation may only be expected on a broad timescale and with a considerable time lag, since warmer oder cooler surface water requires a considerable time span to reach the water table.""",

'result_heatmap': """### Heatmap
Heatmaps are an efficient visual tool to detect patterns in data. For this study, the y-axis represents all stations and the x-axis with a resolution of one year. For each rectangle, the average temperature for the year and station is calculated and a color is assigned to the rectagle representing the temperature value. It must be noted that stations with yearly averages, that were calculated based on few or even a single value, may not represent the true yearly average correctly. This may explain some significant average temperature differences among nearby stations. Stations are ordered by their sampling depth with shallow sampling depths on the top, and deeper sampling depths at the bottom of the plot. The increasing trend is represented by a color change from yellow to blue, as can be observed in station F_1097, or F_1068. The color transition from blue to yellow from top to bottom suggests that the average temperature decreases from shallow towards deeper sampling locations. 
